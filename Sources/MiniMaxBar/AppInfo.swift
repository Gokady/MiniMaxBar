...
enum AppInfo {
    ...
    /// GitHub Releases API:获取最新 release 的元信息(fallback 用)
    static let latestReleaseAPIURL: URL = URL(string:
        "https://api.github.com/repos/\(githubOwner)/\(githubRepo)/releases/latest"
    )!

    /// Sparkle appcast feed URL
    static let appcastURL: URL = URL(string:
        "https://github.com/\(githubOwner)/\(githubRepo)/releases/latest/download/appcast.xml"
    )!
